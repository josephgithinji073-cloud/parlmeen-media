...
var glslPolyfills = {
  bitcast_int_uint: new CodeNode(
    /* glsl */
    "uint tsl_bitcast_uint_to_int ( int x ) { return floatBitsToInt( uintBitsToFloat( x ) ); }"
  ),
...
var RGBShiftNode = class extends TempNode {
  ...
  constructor(textureNode, amount = 5e-3, angle = 0) {
    super("vec4");
    this.textureNode = textureNode;
    this.amount = uniform2(amount);
    this.angle = uniform2(angle);
  }
  /**
  ...
  setup() {
    ...
    const rgbShift2 = Fn2(() => {
      const offset = vec22(cos2(this.angle), sin2(this.angle)).mul(this.amount);
      const cr = sampleTexture(uvNode.add(offset));
      const cga = sampleTexture(uvNode);
      const cb = sampleTexture(uvNode.sub(offset));
      return vec42(cr.r, cga.g, cb.b, cga.a);
    });
    return rgbShift2();
  }
};
var RGBShiftNode_default = RGBShiftNode;
var rgbShift = (node, amount, angle) => nodeObject2(new RGBShiftNode(convertToTexture2(node), amount, angle));
export {
  RGBShiftNode_default as default,
  rgbShift
};
//# sourceMappingURL=three_examples_jsm_tsl_display_RGBShiftNode__js.js.map
